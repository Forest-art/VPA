...
def test(model, dataset):
    ...
    gts_att, gts_obj = [], []
    progress_bar = tqdm.tqdm(
            total=len(dataloader), desc="Evaluate"
        )
    for idx, data in enumerate(dataloader):
        image = data[0].cuda()
        attr_truth, obj_truth, pair_truth = data[1], data[2], data[3]
        att_logits, obj_logits = model(image)
        att_logits, obj_logits = att_logits.detach().cpu(), obj_logits.detach().cpu()
        _, pred_obj = obj_logits.type(torch.float).topk(k=1, dim=1)
        _, pred_att = att_logits.type(torch.float).topk(k=1, dim=1)
        preds_obj.extend(pred_obj.numpy())
        gts_obj.extend(obj_truth.unsqueeze(1).repeat(1,1).numpy())
        preds_att.extend(pred_att.numpy())
        gts_att.extend(attr_truth.unsqueeze(1).repeat(1,1).numpy())
        progress_bar.update()
    progress_bar.close()

    acc_obj = calu(preds_obj, gts_obj)
    acc_att = calu(preds_att, gts_att)
    print("object accuracy is {}, state accuracy is {}".format(acc_obj, acc_att))
...
if __name__ == "__main__":
    ...
    model = VPA(config, attributes=attributes, classes=classes, offset=offset).cuda()
